...
const EXPECTED_POTIONS = [
  ...
  ['copy-and-paste', 'Copy and Paste', 'コピー＆ペースト', 'Board', 'Common', 'Duplicate any common token in your token pouch', 'トークンポーチにある好きなコモントークンを1個複製する。', 'orange'],
  ...
  ['redo', 'Redo', 'やり直し', 'Board', 'Rare', 'You can rearrange your tokens as much as you like this turn.', 'このターン中、トークンを好きなだけ並べ替えることができる。', 'yellow'],
  ...
  ['heavy-weight', 'Heavy Weight', 'ヘビーウェイト', 'Board', 'Rare', 'The nearest player to you (chosen randomly if there are two or more players of equal distance) will have their dice roll halved (rounded up) next turn.', '自分に最も近いプレイヤー（同じ距離に2人以上いる場合はランダムに選ばれる）は、次のターンのサイコロの出目が半分になる（端数切り上げ）。', 'blue'],
  ...
  ['buy-and-sell', 'Buy and Sell', '売買', 'Board', 'Rare', 'Discard four tokens from your token pouch. Get a choice of 2 random tokens and pick one to gain.', 'トークンポーチからトークンを4個捨てる。ランダムに選ばれた2個のトークンから1個を選んで獲得する。', 'orange'],
  ['cauldron', 'Cauldron', '大釜', 'Board', 'Common', 'Pick one of 3 potions to gain.', '3つのポーションから1つを選んで獲得する。', 'red'],
  ['storm-master', 'Storm Master', '嵐の支配者', 'Board', 'Rare', 'Roll the dice for your turn. If even all others players cannot move on their next turn (they must repeat the square they are currently on), if odd you may not move this turn and your turn is over.', 'このターンのサイコロを振る。出目が偶数なら、自分以外のすべてのプレイヤーは次のターンに移動できず、現在いるマスにとどまる。奇数なら、自分はこのターン移動できず、そのままターンを終了する。', 'red'],
...
describe('potion definitions', () => {
  test('matches the complete revamped source-of-truth table', () => {
    ...
    expect(POTION_DEFINITIONS).toHaveLength(26);
    expect(new Set(POTION_DEFINITIONS.map(({ id }) => id)).size).toBe(26);
  });
