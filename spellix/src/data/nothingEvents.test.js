...
test('defines only the five required Nothing-event environments and exact text', () => {
  expect(NOTHING_EVENT_ENVIRONMENTS).toEqual([
    {
      background: 'fields',
      id: 'field',
      label: 'Field',
...
test('provides exact localized actions and safe environment lookup', () => {
  expect(NOTHING_EVENT_TRIGGER_TEXT).toEqual({
    en: 'Trigger Nothing Event',
    jp: '\u4f55\u3082\u306a\u3044\u30a4\u30d9\u30f3\u30c8\u3092\u767a\u751f\u3055\u305b\u308b',
  });
  expect(NOTHING_EVENT_CONTINUE_TEXT).toEqual({
    en: 'Continue',
    jp: '\u7d9a\u3051\u308b',
  });
  expect(getNothingEventForEnvironment('stream')).toBe(
